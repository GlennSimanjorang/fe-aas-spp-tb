        ...
        @else
            ...
            <form action="{{ route('pembayaran.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select id="bill_id" name="bill_id" 
                            ...
                            @foreach($list_bills as $bill)
                                ...
                                <option value="{{ $bill['id'] }}" {{ old('bill_id') == $bill['id'] ? 'selected' : '' }}>
                                    ({{ $bill['student_nisn'] ?? 'N/A' }}) {{ $bill['student_name'] ?? 'N/A' }} - Sisa: Rp{{ number_format($outstanding_amount, 0, ',', '.') }} ({{ $bill['payment_category_name'] ?? 'Tagihan' }})
                                </option>
                            @endforeach
                        </select>
                        @error('bill_id')
                    ...
                    {{-- Input Jumlah Pembayaran (Ganti dari 'jumlah' ke 'amount_paid') --}}
                    <div>
                    ...
                    {{-- Input Metode Bayar (Ganti dari 'metode_bayar' ke 'payment_method') --}}
                    <div>
                        ...
                        <select id="payment_method" name="payment_method" 
                            ...
                            {{-- Metode Virtual Account TIDAK disarankan untuk input manual karena VA diurus Midtrans --}}
                        </select>
